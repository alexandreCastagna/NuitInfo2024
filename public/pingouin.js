...
let SCOOOOOOOOOOOOOOORE = 0;
let SCOOOOOOOOOOOOOOOREMAX = 0
helium()
function j_ai_reson(num_reponse) {
    ...
    fetch('/question_api')
        ...
        .then((data) => {
            ...
            if(question_actuellel['numReponse'] == num_reponse)
            {
                SCOOOOOOOOOOOOOOORE+= 500; 
            }
            SCOOOOOOOOOOOOOOOREMAX+= 500;
    ...
    function switch_du_grizly(){
        ...
        if (question_actuellel == null){
            alert("vous avez eu "+SCOOOOOOOOOOOOOOORE+" sur "+SCOOOOOOOOOOOOOOOREMAX+" points dans la categorie")
            let pre_pingouin = document.location.search;
            let canard = new URLSearchParams(pre_pingouin);
            let pingouin = canard.get('num_tortue')
